...
public:
    ...
    {
        assert(lft.isEmpty() || lft.root().getIndex() < val.getIndex());
        assert(rgt.isEmpty() || val.getIndex() < rgt.root().getIndex());
    }
  ...
  bool find(T x, T &foundValue) const {
        ...
        if (x.getIndex() < y.getIndex())
          ...
          foundValue.setIndex(y.getIndex());
          foundValue.setKeyword(y.getKeyword());
          return true;
        }
    }
